...
export enum InputEventType {
  MOUSE_EVENT, // 总类，表示鼠标事件
  MOUSEDOWN, //鼠标按下事件
  MOUSEUP, //鼠标弹起事件
  MOUSEMOVE, //鼠标移动事件, move事件发生在当前鼠标指针之下的ISprite对象
  MOUSE_DRAG, //鼠标拖动事件, drag事件发生在mouseDown选中的ISprite对象上
  KEYBOARD_EVENT, //键盘事件
  KEYUP, //总类，键按下事件
  KEYDOWN, //键弹起事件
  KEYPRESS, //按键事件
...
export class Application implements EventListenerObject {
  ...
  public handleEvent(evt: Event): void {
    switch (evt.type) {
      case "mousedown":
        ...
        this.onMouseDown(
          this._toCanvasMouseEvent(evt, InputEventType.MOUSEDOWN)
        );
        break;
      case "mouseup":
        this._isMouseDown = false;
        this.onMouseUp(this._toCanvasMouseEvent(evt, InputEventType.MOUSEUP));
        break;
      case "mousemove":
        // 如果isSupportMouseMove为true，才会每次鼠标移动触发mouseMove事件
        if (this.isSupportMouseMove) {
          this.onMouseMove(
            this._toCanvasMouseEvent(evt, InputEventType.MOUSEMOVE)
          );
        }
        // 同时，如果当前鼠标任意一个键处于按下状态并拖动时，触发drag事件
        if (this._isMouseDown) {
          this.onMouseDrag(
            this._toCanvasMouseEvent(evt, InputEventType.MOUSEDRAG)
          );
        }
        break;
      case "keypress":
        this.onKeyPress(
          this._toCanvasKeyBoardEvent(evt, InputEventType.KEYPRESS)
        );
        break;
      case "keydown":
        this.onKeyDown(
          this._toCanvasKeyBoardEvent(evt, InputEventType.KEYDOWN)
        );
        break;
      case "keyup":
        this.onKeyUp(this._toCanvasKeyBoardEvent(evt, InputEventType.KEYUP));
        break;
    }
  }
  ...
  private _toCanvasMouseEvent(
    ...
    if (type === InputEventType.MOUSEDOWN && event.button === 2) {
      this._isRightMouseDown = true;
    } else if (type === InputEventType.MOUSEUP && event.button === 2) {
      this._isRightMouseDown = false;
    }

    let buttonNum: number = event.button;

    if (this._isRightMouseDown && type === InputEventType.MOUSEDRAG) {
      buttonNum = 2;
    }
